...
void Set (int matrice[][M], int n, int m, int riga, int colonna, int val);
...
int main()
{
    ...
    while(flag==1&&scanf("%d%c",&numero,&spazio)!=EOF){
        ...
        switch(spazio){
            ...
            case '\n':
                i++;
                if(j>num_colonne) num_colonne=j+1;
                j=0;
                break;
    ...
    if(flag){
        ...
        do{ printf("Dimmi riga e colonna da dove iniziare ad azzerare la sottomatrice da te scelta: ");
            ...
            if(riga>num_righe||colonna>num_colonne||riga<0||colonna<0){
                ...
                printf("Si partira' dunque dal numero %d.\n",matrice[riga][colonna]);
                flag=1;
            }
            }
        while(!flag);
        ...
        Set(matrice,n,m,riga,colonna,matrice[riga][colonna]);
