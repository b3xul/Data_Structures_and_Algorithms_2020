...
//Item esercizio_read(FILE* fp);
//void greedy(Elementi e);
//
//int main()
//{
//    Elementi e=elementi_read();
//    MergeSort(e.elementi,e.n);
//
//    greedy(e);
//
//    return 0;
...
//
//void greedy(Elementi e){
//    int i,j,tipo=-1,d_tot=0,pos,exit;
//    float tot;
//    int d_diag[3]={0};
//    int programma[MAX_DIAG][MAX_ES];
//
...
//
//    for(i=0;i<e.n;i++){
//        if((e.elementi[i].d_ingresso==1) && (e.elementi[i].requisito_precedenza==0) && e.elementi[i].tipo!=0 && e.elementi[i].difficolta<=DD){  //primo elemento ha direzione di ingresso= di spalle
//            programma[0][0]=i;
//            d_diag[0]=e.elementi[i].difficolta;
//            d_tot+=e.elementi[i].difficolta;
//            tipo=e.elementi[i].tipo;
//            break;
//        }
//    }
//    if(tipo==-1){
//        printf("Nessuna soluzione valida.\n");
//        return;
//    }
//    tipo=-1;
//
//    for(i=0;i<e.n;i++){
//        if((e.elementi[i].d_ingresso==1) && (e.elementi[i].requisito_precedenza==0) && e.elementi[i].tipo!=0 && e.elementi[i].tipo!=tipo && e.elementi[i].difficolta<=DD && d_tot+e.elementi[i].difficolta<=DP ){  //primo elemento ha direzione di ingresso= di spalle
//            programma[1][0]=i;
//            d_diag[1]=e.elementi[i].difficolta;
//            d_tot+=e.elementi[i].difficolta;
//            tipo=e.elementi[i].tipo;
//            break;
//        }
...
//    pos=0;
//    for(i=e.n-1; i>=0 ;i--){
//        if(pos==0 && (e.elementi[i].d_ingresso==1) && (e.elementi[i].requisito_precedenza==0) && e.elementi[i].difficolta<=DD && d_tot+e.elementi[i].difficolta<=DP ){
//            programma[2][pos++]=i;
//            d_diag[2]+=e.elementi[i].difficolta;
//            d_tot+=e.elementi[i].difficolta;
//            tipo=e.elementi[i].tipo;
//        }
//    }
...
//    while(!exit){
//        for(i=e.n-1; i>=0 ;i--){
//            if(e.elementi[programma[2][pos-1]].d_uscita == e.elementi[i].d_ingresso && d_diag[2]+e.elementi[i].difficolta<=DD && d_tot+e.elementi[i].difficolta<=DP ){
//                programma[2][pos++]=i;
//                d_diag[2]+=e.elementi[i].difficolta;
//                d_tot+=e.elementi[i].difficolta;
//                if(pos==MAX_ES)
...
//        for(j=0;j<MAX_ES;j++){
//            if(programma[i][j]!=-1){
//                ITEMshow(e.elementi[programma[i][j]]);
//                tot+=e.elementi[programma[i][j]].valore;
//            }
//        }
//    }
//
...
//    fscanf(fp,"%d",&e.n);
//
//    e.elementi=(Item*)malloc(e.n*sizeof(Item));
//    for(i=0;i<e.n;i++)
//        e.elementi[i]=ITEMscan(fp);
//
//    return e;
//}
