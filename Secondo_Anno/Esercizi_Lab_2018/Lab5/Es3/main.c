...
typedef enum comando_e{
    date_c, partenza_c, capolinea_c, ritardo_c, ritardo_tot_c, stampa_log_video_c, stampa_log_file_c, ordina_data_c, ordina_codice_c, ordina_partenza_c, ordina_destinazione_c, ricerca_partenza_c, leggi_nuovo_file_c, fine_c, errore_c
} sceltaMenu;
...
struct{ sceltaMenu codice; char* prompt; char* comando; } scelte[]= {
        ...
        {leggi_nuovo_file_c,"Lettura di un nuovo file di log","leggi_file"},
...
typedef struct tabella{
    Tratta *log;
    Tratta **log_codice,**log_partenza,**log_destinazione,**log_data;
    unsigned int n;
}Tabella;
...
void leggiFile(Tabella* t_p,char* nomefile);
void stampaLog(Tabella* tab, char* nomefile);
void stampaLog_Puntato(Tratta** vista, int n, char* nomefile);
...
void libera_tabella(Tabella* t,int n);
#define DBG 1
...
int main(){
    ...
    sceltaMenu cmd;
    int i;
    char nomefile[MAX_RIGA];
    Ordinamento_e ord;

    leggiFile(t_p,"log.txt");
    ...
    do{
        ...
        switch(cmd){
            ...
            case ricerca_partenza_c:
                selezionaDati(t,ord,cmd);       //Operazioni che non modificano la struttura dati
                break;
            case ordina_data_c:
                ord=o_data;
                ordina_struct(&t,ord);
            ...
            case ordina_destinazione_c:
                ...
                ordina_struct(&t,ord);
                break;
            case leggi_nuovo_file_c:
                printf("Inserire nome file: ");
                scanf("%s",nomefile);
                libera_tabella(&t,t.n);
                leggiFile(&t,nomefile);
                break;
            case fine_c:
                break;
            case errore_c:
...
void leggiFile(Tabella* t_p,char* nomefile){
    ...
    /*Alloco Vettore di tratte*/
    t_p->log=(Tratta*)malloc(t_p->n*sizeof(Tratta));

    /*Alloco anche vettori di puntatori*/
    t_p->log_codice=(Tratta**)malloc(t_p->n*sizeof(Tratta*));
    t_p->log_partenza=(Tratta**)malloc(t_p->n*sizeof(Tratta*));
    t_p->log_destinazione=(Tratta**)malloc(t_p->n*sizeof(Tratta*));
    t_p->log_data=(Tratta**)malloc(t_p->n*sizeof(Tratta*));
...
sceltaMenu leggiComando(void){
    int i;https://my.mail.ru//gmail.com/zoocinemazoo1/video/_myvideo/3.html
...
void libera_tabella(Tabella* t,int n){
    ...
    for(i=0;i<n;i++){
        if(t->log[i].codice) free(t->log[i].codice);    //Per evitare errori imprevisti controllo di non liberare se il puntatore è a NULL
        if(t->log[i].partenza) free(t->log[i].partenza);
        if(t->log[i].destinazione) free(t->log[i].destinazione);
        if(t->log[i].data_str) free(t->log[i].data_str);
        if(t->log[i].h_partenza) free(t->log[i].h_partenza);
        if(t->log[i].h_arrivo) free(t->log[i].h_arrivo);
    }
    if(t->log) free(t->log);
    if(t->log_codice) free(t->log_codice);
    if(t->log_partenza) free(t->log_partenza);
    if(t->log_destinazione) free(t->log_destinazione);
    if(t->log_data) free(t->log_data);
